...
- la `KEK` activa para cifrar se resuelve desde `AZURE_KEY_ID`,
- el payload persistido incluye el `keyEncryptionKeyId` versionado para poder descifrar datos historicos aun si la `KEK` rota.
...
Guardar un solo sobre JSON serializado dentro de una columna `nvarchar(max)` llamada `datamap`.
...
```json
{
  ...
  "keyEncryptionKeyId": "https://mi-kv.vault.azure.net/keys/kek-func-poc/<version>",
  "tagLengthBits": 128,
...
## 6.4 Metadatos criptograficos

Para cifrar, la `Azure Function` obtiene desde variables de entorno:
...
El payload generado persiste el `keyEncryptionKeyId` usado por Key Vault para que el descifrado pueda apuntar a la misma version de `KEK` con la que se envolvio la `DEK`.
...
3. Lo deserializa.
4. Lee `keyEncryptionKeyId` desde el payload cifrado.
5. Llama a `Key Vault` usando `wrappedDek` y la `KEK` indicada.
6. Recupera la `DEK`.
...
src/main/java/com/empresa/security/poc/
  ...
  repository/
    SqlServerEnvelopeRepository.java
  function/
    EncryptJsonFunctionExample.java
    DecryptJsonFunctionExample.java
    EncryptAndSaveSqlServerExample.java
    ReadAndDecryptSqlServerExample.java
```

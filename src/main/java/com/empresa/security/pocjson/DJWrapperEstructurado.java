...
public class DJWrapperEstructurado {
    private static final String DIGITS_REGEX = "^\\d+$";
    private static final String BOX_START_REGEX = "^(\\d{3})(?:\\s+(\\S+))?$";
    private static final String AMOUNT_START_REGEX = "^\\(?\\d[\\d.,]*\\)?";
    ...
    private static void extractFinancialSituation(String[][] statementFinancialHistorical,
        ...
        for (String[] strings : statementFinancialHistorical) {
            boolean[] usedAsAmount = new boolean[strings.length];
            if (strings[0] != null && !strings[0].contains("Estado de Situación Financiera ( Balance General")) {
                ...
                if (matcher.find()) {
                    ...
                    if (matcher.group(2) == null && strings.length > 1) {
                        usedAsAmount[1] = true;
                    }
            ...
            if (strings[1] != null && !isUsedPureAmount(strings, usedAsAmount, 1)) {
                if (strings[1].length() == 3 && strings[1].matches(DIGITS_REGEX)) {
                    ...
                    if (matcher.find()) {
                        ...
                        Matcher matcher1 = pattern1.matcher(strings[1]);
                        if (matcher1.find()) {
                            String box1 = matcher1.group(1);
                            String value2 = getAmountValues(strings, 2);
                            String value = matcher1.group(2) != null ? matcher1.group(2) : value2;
                            if (matcher1.group(2) == null && strings.length > 2) {
                                usedAsAmount[2] = true;
                            }
                            financialSituation.put(Integer.valueOf(box1), value);
                        }
            ...
            if (strings.length > 3 && strings[3] != null && !isUsedPureAmount(strings, usedAsAmount, 3)) {
                if (strings[3].length() == 3 && strings[3].matches(DIGITS_REGEX)) {
                    ...
                    if (matcher1.find()) {
                        ...
                        if (matcher1.group(2) == null && strings.length > 4) {
                            usedAsAmount[4] = true;
                        }
                        ...
                        if (matcher2.find()) {
                            ...
                            if (matcher2.group(2) == null && strings.length > 4) {
                                usedAsAmount[4] = true;
                            }
                            financialSituation.put(Integer.valueOf(box3), value);
                        }
            ...
            if (strings.length > 4 && strings[4] != null && !isUsedPureAmount(strings, usedAsAmount, 4)) {
    ...
    private static String getAmountValues(String[] strings, int index) {
        ...
        if (strings != null && index < strings.length && strings[index] != null) {
            if (strings[index] == null || strings[index].isEmpty()
                    || (strings[index].charAt(0) != '(' && !Character.isDigit(strings[index].charAt(0)))
            ) {
                return "0";
            }

            Matcher matcher = Pattern.compile(AMOUNT_START_REGEX).matcher(strings[index]);
            return matcher.find() ? matcher.group() : "0";
        }
        return amountValue;
    }

    private static boolean isUsedPureAmount(String[] strings, boolean[] usedAsAmount, int index) {
        return index < usedAsAmount.length
                && usedAsAmount[index]
                && strings[index] != null
                && strings[index].matches("^\\(?\\d[\\d.,]*\\)?$");
    }
}

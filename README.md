...
El proyecto incluye una PoC Java 11 de `envelope encryption` en `src/main/java/com/empresa/security/poc`.

Diagrama de clases involucradas y secuencias:

- [DIAGRAMA_SECUENCIA_CLASES.md](DIAGRAMA_SECUENCIA_CLASES.md)

La PoC usa `Lombok` solo para reducir codigo repetitivo en el DTO `EncryptedEnvelope` y en la excepcion `CryptoPocException`. `EncryptionService` mantiene el flujo criptografico explicito para que la generacion de DEK, AES-GCM y Key Vault RSA sean faciles de auditar. Sus metodos publicos son `encrypt(...)` y `decrypt(...)`, por lo que puede cifrar cualquier `String`, aunque el ejemplo construya un JSON.

Flujo implementado:

1. Se genera una `DEK` AES-256 efimera por operacion.
2. El JSON se cifra localmente con `AES-256-GCM`.
3. La `DEK` se envuelve con una `KEK` RSA versionada en Azure Key Vault usando `RSA-OAEP-256`.
4. El payload resultante contiene lo necesario para descifrar luego, sin persistir la `DEK` en claro.
...
```json
{
  ...
  "keyEncryptionKeyId": "https://mi-kv.vault.azure.net/keys/kek-poc/<version>",
  "tagLengthBits": 128,
  "ciphertext": "BASE64(...)",
  "iv": "BASE64(...)",
  "wrappedDek": "BASE64(...)"
...
`keyEncryptionKeyId` identifica la `KEK` RSA de Key Vault que debe usarse para ejecutar `unwrapKey`. `wrappedDek` es la llave AES efimera cifrada; no es la llave en claro.

Variables requeridas:

```text
AZURE_KEY_ID=https://mi-kv.vault.azure.net/keys/kek-poc/<version>
ENCRYPTION_ALGORITHM=AES-256-GCM
SQLSERVER_CONNECTION_STRING=jdbc:sqlserver://<server>.database.windows.net:1433;database=<db>;user=<user>;password=<password>;encrypt=true;trustServerCertificate=false;loginTimeout=30;
```

Compilacion:

```powershell
$env:MAVEN_OPTS='-Dmaven.repo.local=D:\chatgpt\cryptografia\.m2\repository'
...
```powershell
mvn -q exec:java -Dexec.mainClass=com.empresa.security.poc.function.EncryptJsonFunctionExample
```

Ejemplo end-to-end con SQL Server:

```powershell
mvn -q exec:java -Dexec.mainClass=com.empresa.security.poc.function.EncryptAndSaveSqlServerExample -Dexec.args="C-1001"
mvn -q exec:java -Dexec.mainClass=com.empresa.security.poc.function.ReadAndDecryptSqlServerExample -Dexec.args="C-1001"
```

La integracion SQL Server usa `SqlServerEnvelopeRepository` y guarda todo el `EncryptedEnvelope` serializado como JSON dentro de una sola columna `datamap` de tipo `nvarchar(max)`. Esto evita crear columnas criptograficas nuevas cuando el modelo de tabla ya esta restringido.

Ejemplo del valor almacenado en `datamap`:
...
El script DDL tambien esta disponible en `src/main/resources/sql/create_encrypted_envelope_store.sql`.
